...
const SortPopup = memo(({ items, sortByType, onClickSort }) => {
  ...
  const activeSort = items.find((obj) => obj.type === sortByType).name;
              ...
              items.map((obj, id) => {
                return (
                  <li
                    ...
                    key={`${obj.type}_${id}`}
                    className={obj.type === sortByType ? 'active' : null}
                  >
                    {obj.name}
                  </li>
                );
...
SortPopup.propTypes = {
  ...
  sortByType: PropTypes.string.isRequired,
